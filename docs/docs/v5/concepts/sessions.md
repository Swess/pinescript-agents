...
Session strings containing from-to start times and day
...
Session input section for more information).
Session states built-in variables such as
session.ismarket
...
Notice!The default days are: 1234567, which is different in Pine Script
...
timeframe, whatever that is. The function returns true when:

The chart uses an intraday timeframe (seconds or minutes).
The script isn’t on the chart’s first bar, which we ensure with
...
regularSessionData = request.security(t, timeframe.period, close, barmerge.gaps_on)
plot(regularSessionData, style = plot.style_linebr)         Previous       Repainting      Next   Strategies              On this pageIntroductionSession stringsSession string specificationsUsing session stringsSession statesUsing sessions with request.security()
```