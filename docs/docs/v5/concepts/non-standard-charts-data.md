...
direction changes when the price changes1 beyond a predetermined
amount. The
ticker.kagi()
function creates a ticker id which can be used with
request.security()
...
```
​ticker.pointfigure()​
Point and Figure (PnF) charts only plot price movements1, without
...
they were generated from tick data. ↩ ↩2 ↩3 ↩4