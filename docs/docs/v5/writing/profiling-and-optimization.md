...
Notice!As with profiling tools for other languages, the Pine Profiler wraps a
script and its significant code with
extra calculations required to collect performance data. As such, a script’s
...
precise performance measurements.Furthermore, the Profiler cannot collect and display individual
...
Notice!When the local scopes of a script’s
...
user-defined functions will not include the time spent on the request.*()
calls. See the
...
here.
Notice!The Profiler cannot collect individual performance data for any
...
Notice!It’s often wise to profile each configuration more than once to
...
Notice!Whether profiling a script over a single run or multiple, it’s crucial
...
Notice!Before looking for ways to optimize a script,
profile it to gauge its performance and identify the critical code
...
efficiency.
Notice!Before identifying ways to optimize a loop, we recommend searching for
ways to
eliminate it first. If no solution exists that makes the loop
...
Notice!When explicitly defining a buffer size for a problematic historical
reference with
max_bars_back(),
...
user-specified size is insufficient.Another consideration when explicitly defining buffer sizes is that the
...
Profiler, experiment with different ways to limit its executions
