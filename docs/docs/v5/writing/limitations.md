...
Events triggering the execution of a script for a list of the events triggering the execution of a
...
methods,
conditional structures it
uses, cannot exceed 550.
...
if
structures. The total scope count for this script is 301 (1 global
scope + 300 local scopes):
```pinescript
//@version=5
...
if blocks
within a
user-defined function, as the scope of a function counts as one embedded scope:
```pinescript
//@version=5
indicator("Scopes demo")
...
using Deep Backtesting, the limit is 1,000,000.         Previous       Publishing scripts           On this pageIntroductionTimeScript compilationScript executionLoop executionChart visualsPlot limitsLine, box, polyline, and label limitsTable limitsrequest.\*() callsNumber of callsIntrabarsTuple element limitScript size and memoryCompiled tokensVariables per scopeScope countCompilation request sizeCollectionsOther limitationsMaximum bars backMaximum bars forwardChart barsTrade orders in backtesting