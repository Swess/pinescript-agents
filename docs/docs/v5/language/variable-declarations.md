...
plotColor = if close > open
    ...
    color.red
Notice!The above statements all contain the = assignment operator because
...
Variable reassignment section for details.
...
<type>    int | float | bool | color | string | line | linefill | label | box | table | array<type> | matrix<type> | UDF
...
behavior in realtime.         Previous       Operators      Next   Conditional structures              On this pageIntroductionInitialization with naTuple declarationsUsing an underscore (_) as an identifierVariable reassignmentDeclaration modesOn each barvarvarip