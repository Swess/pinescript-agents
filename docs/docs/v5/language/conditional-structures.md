...
x = if close > open
    ...
    "open"         Previous       Variable declarations      Next   Loops              On this pageIntroductionif structureif used for its side effectsif used to return a valueswitch structureswitch with an expressionswitch without an expressionMatching local block type requirement