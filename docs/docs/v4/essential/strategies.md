...
NoticeWhen applying strategies to
non-standard types of charts (Heikin Ashi, Renko, etc.), it is very important to realize
...
If the bar’s high is closer to bar’s open than the bar’s low, the
broker emulator assumes that intrabar price was moving this way:
open → high → low → close.
If the bar’s low is closer to bar’s open than the bar’s high, the
broker emulator assumes that intrabar price was moving this way:
open → low → high → close.
The broker emulator assumes that there are no gaps inside bars,
...
condition is met.         Previous       Non-standard chart types data      Next   Indicator repainting              On this pageOverviewA simple strategy exampleApplying a strategy to a chartBacktesting and forwardtestingBroker emulatorOrder placement commandsClosing market positionOCA groupsRisk managementCurrencyLeverage