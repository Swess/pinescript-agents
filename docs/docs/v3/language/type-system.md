...
NoticeThe operator [] also returns a value of a series type.
...
number itself.
There is a string type which is used for the indicators names, inputs,
line graphs, names of tickers, resolutions, trade sessions, etc.
...
Finally, there is also a void type in Pine Script. Some functions with
side effect return void result. For example a
strategy.entry.
Type cast rules
...
NoticePine type system was changed in version 3. See
Release notes. See also type casts
graph for version 2:         Previous       Lines wrapping      Next   Literals