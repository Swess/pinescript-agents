    ...
    ud = isEqual ?
           ...
           isGrowing ?
               ...
               (nz(ud[1]) >= 0 ?
                   -1 :
                   nz(ud[1])-1)
This rule also applies to comments. Don’t use comments combined with
lines wrapping. Following code would NOT compile:
```pinescript
//@version=2
study("My Script")
c = open > close ? red :
...
limitation is inconvenient… We hope it could be removed in future
Pine releases.         Previous       Identifiers      Next   Type system