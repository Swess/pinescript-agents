...
For up-to-date documentation, see the latest version.
      Literals
Fixed values assigned with immediate values (e.g., 10, 3.14,
"value"), which may not be altered by the script, are called literals.
...
high, low, close, volume, time, hl2, hlc3, ohlc4). These
...
delimiter (the symbol .) and/or the symbol e (which means “multiply
...
NoticeIt’s possible to use uppercase E instead of lowercase e.
Boolean literals
...
Single or double quotation marks are completely the same --- you may
use what you prefer. The line that was written with double quotation
marks may contain a single quotation mark, just as a line that is
written with single quotation marks may contain double quotation marks:
"It's an example"
'The "Star" indicator'
...
component. Each component value is a hexadecimal number between 00 and
FF (0 and 255 in decimal).
Fourth pair of digits is optional. When set, it specifies the alpha
(opacity) component which value is also between 00 (fully transparent)
and FF (fully opaque). Examples:
#000000                // black color
#FF0000                // red color
#00FF00                // green color
#0000FF                // blue color
...
NoticeWhen using hexadecimal figures it’s possible to use them in either
upper or lowercase.
