...
two horizontal lines (created with hline). The following example
...
Please note that fill annotation is not designed to be used with
plot and hline arguments simultaneously. As a workaround you may
replace hline call with a plot(<constant number>). For example:
...
color = close >= open ? green : red. Example:
```pinescript
//@version=2
