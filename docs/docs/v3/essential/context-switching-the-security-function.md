...
following script will display the close price of the IBM symbol but on
a 15 resolution.
...
security
function.
Here is an example that shows the behavioral difference of the security
function on a 5 minute chart:
```pinescript
//@version=3
...
Actually the highest supported minute resolution is “1440”
(which is a number of minutes in 24 hours). ↩

Requesting data of "1h" or "1H" resolution would result in an
error. Use "60" instead. ↩

          Next   Bar states. Built-in variables `barstate.*`              On this pageOverviewDetailed descriptionBarmerge gaps and lookaheadUnderstanding lookaheadRequesting data of a lower timeframeFootnotes