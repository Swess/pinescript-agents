...
seen from the screenshot, the value time on the last bar is equal to
...
colored over with grey). The function time returns the time of the bar
start in milliseconds UNIX time or na value if the bar is located
outside the given trade session (09:30-16:00 in our example). time
accepts two arguments, the first is resolution, the bars of which are
...
“0900-1600,1700-2000”
     a session that begins at 9:00 with a break at 16:00 until 17:00 and
ending at 20:00.
“2000-1630”
...
“0000-0000:23456”
     same as “0000-0000”, Monday to Friday session that starts every
day at 00:00 and ends at 00:00 of the next day.
“1700-1700”
     is an overnight session. Monday session starts at Sunday, 17:00,
and ends at Monday, 17:00. Also, only on Monday—Friday.
“1000-1001:26”
     is a weird session, that lasts only one minute on Mondays (2), and
one minute on Fridays (6).
Session specification, which is being passed to the function time, is
...
The function is_newbar similar to the previous example can be used in
...
```
Pay attention to the variables highTimeFrame and sessSpec. They have
...
functions which make it possible to use time in various cases of the
script logic.
...
except for the time and timenow variables which return time in UTC
timezone.
Footnotes

UNIX time is measured in seconds. Pine Script uses UNIX time
