...
is incorrect. Pine of version 2 is better at explaining errors of this
kind. So you can try to switch to version 2 by adding a
...
there is no support for series of string type in Pine Script. So this
call is forbidden.
...
```
    t
plot(f(301))         Previous       HOWTOs      Next   Pine Script v2 preprocessor              On this pageScript requesting too many securitiesScript could not be translated from: nullline 2: no viable alternative at character ’$’Mismatched input <…> expect <???>Cannot call operator ?: with arguments (series, string, string)Loop is too long (> 200 ms)Script has too many local variablesPine cannot determine the referencing length of a series. Try using max_bars_back in the study or strategy function