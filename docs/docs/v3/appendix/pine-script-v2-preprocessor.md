...
code, such as function body, if or for body. Every tab or four
spaces are replaced with token |INDENT|.
Add |B| and |E| tokens. They indicate line begin and line end.
Replace empty lines with |EMPTY| tokens.
Join lines that represent one splitted statement.
Add code block tokens (|BEGIN| — beginning of the block,
|END| — end of the block, |PE| — possible end of the
block).
...
|EMPTY|"
After step 7). Note that line with plot(c, title= has been joined with
the next line:
"|EMPTY|
|B|study('Preprocessor example')|E|
|B|fun(x, y) =>|E|
...
"         Previous       Pine compilation errors      Next   Pine Script v2 lexer grammar