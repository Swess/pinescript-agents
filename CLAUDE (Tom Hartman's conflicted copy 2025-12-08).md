...
### **CRITICAL: Always Optimize After Code Changes**
**After ANY code modification to a Pine Script file, you MUST:**
1. Use the `pine-optimizer` agent to review the changes
2. Pass the full context of what was changed and why
3. Let the optimizer analyze for:
   - Performance improvements
   - User experience enhancements
   - Visual presentation
   - Best practices compliance

**This is mandatory, not optional.** The optimizer ensures professional quality and catches issues early.

### For Simple Requests
When users ask for simple indicators or strategies, you can handle these directly or delegate to appropriate agents:
- "Create an RSI indicator" → Use pine-developer → **Then pine-optimizer**
- "Debug my script" → Use pine-debugger → **Then pine-optimizer**
...
- **Final step is always pine-optimizer**
...
- Code implementation → pine-developer → **pine-optimizer**
- Error fixing → pine-debugger → **pine-optimizer**
- Performance testing → pine-backtester
- UX improvement → pine-optimizer
- Publishing preparation → pine-publisher
- Complex multi-step projects → pine-manager → **pine-optimizer**
...
6. **Inline Inputs**: When using enable/disable booleans with associated value fields:
   - Use `inline` parameter to group them on the same line
   - Place the label on the boolean field, leave the value field label empty
   - Example:
     ```pine
     showUpper = input.bool(true, "Upper Deviation", inline="upper")
     upperValue = input.float(2.0, "", inline="upper")
     ```
